# 閩拚輸入法（RIME BP）打包作業程序書

## 概述

完成開發，即將發行之【閩拼輸入法】，循此作業程序書之【工作流程】依次執行，
完成【閩拚輸入法】之發行套件打包：
...
```bash
rime-bp/
├── release-include.txt         # 指定【發行套件包】應包含之檔案清單
├── config/
│   └── default.custom.yaml     # 新的 RIME 設定檔
├── tools/
│   ├── build_system.py         # 【發行套件包】之建置執行程式
│   ├── rime_installer.py       # 輪入法之安裝程式
│   └── test_xlit_rules.py      # 輸入法安裝結果驗證工具
└── release/                    # 發布目錄
    ├── rime_files/             # 發行需打包之所有檔案
    ├── RIME_Installer.exe      # 安裝程式
    ├── RIME_Validator.exe      # 驗證工具
...
執行以下 Python 工具程式：build_system.py ，用以
建置【閩拚輸入法】之發行套件包，以供使用者下載及安裝。
...
- 清理前次發行作業用所有檔案（目錄路徑： release/rime_files/ ）
- 安裝必要的依賴套件
