            ...
            for line in f:
                ...
                if line and not line.startswith('#'):
                    ...
                    if src_file.exists():
                        ...
                        copied_count += 1
        print(f"✅ 已複製 {copied_count} 個 RIME 檔案到 rime_files 目錄")
...
def main():
    ...
    if not rime_installer_path:
        print("❌ 找不到 rime_installer.py，請檢查檔案是否存在")
        print(f"   已檢查位置:")
        for path in possible_locations:
            print(f"   - {path}")
        return False

    print(f"✅ 找到 rime_installer.py: {rime_installer_path}")
    print(f"📍 工作目錄: {working_dir}")

    # 切換到正確的工作目錄
    original_cwd = Path.cwd()
    os.chdir(working_dir)
    print(f"🔄 已切換工作目錄到: {Path.cwd()}")
